...
class Gps_Tracker_Endpoint {
    ...
    public function process_gpstracker_query() {
        ...
        switch ( $wp_query->query_vars['gpstracker'] ) {
            case 'nonce':
                $phone_number = isset($wp_query->query_vars['phonenumber']) ? $wp_query->query_vars['phonenumber'] : '0';
                ...
                if ( preg_match($session_id_pattern, $session_id) && preg_match($session_id_pattern, $phone_number) ) {
                    $app_id = get_option('gpstracker_app_id');
                    ...
                    if ( ! $app_id ) {
                        ...
                        $gps_action = 'a';  
                        add_option( 'gpstracker_app_id', $phone_number, '', 'yes' );
                        $gps_nonce = wp_create_nonce($session_id);
                        echo $gps_nonce;   
                    } else {
                        if ( $phone_number == $app_id ) {
                            $gps_action = 'b';
                            $gps_nonce = wp_create_nonce($session_id); 
                            echo $gps_nonce;
                        } else {
